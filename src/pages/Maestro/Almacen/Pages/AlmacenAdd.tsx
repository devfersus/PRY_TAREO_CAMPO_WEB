...
import { createAlmacenForm } from '../api/getAlmacenes.action';

interface Props {
    onAddAlmacen: (almacen: IAlmacen) => void;
    onCancel?   : () => void;
}

export const AddAlmacenForm = ({ onAddAlmacen, onCancel }: Props) => {
    const [estado, setEstado] = useState(true);

    const [_state, formAction, isPending] = useActionState(
        async (prevState: unknown, queryData: FormData) => {
            const almacen = await createAlmacenForm(prevState, queryData);
            onAddAlmacen(almacen);
        },
        null
    );
